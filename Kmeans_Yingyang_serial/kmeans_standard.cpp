...
// helper function to normalize a vector vectors
std::vector<double>& normalize_vector(std::vector<double>& a) {
  double d = 1.0/l2_norm(a);
  for (unsigned int i = 0;i < a.size(); ++i) {
    a[i] *= d;
  }
  return a;
...
void updatecenter(std::vector<vertex> &points, std::vector<cluster> &clusters)
{
  ...
  for (unsigned int i = 0; i < num_clusters; ++i) {
   ...
   normalize_vector(clusters[i].center);
  }
}
...
void kmeans_standard(RepsSpace rs){
  ...
  #ifdef printtrue
  std::cout << "Converge with itertions " << iteration_count << std::endl;
  #endif
  #ifdef debugtrue
  for(unsigned int i = 0; i < points.size(); i++){
      //std::cout << "point[" << i << "]= ("<< points[i].point << ").bestcenter "<< points[i].best_cluster << " Distance: " << points[i].best_distance << std::endl;
      std::cout << "point[" << i << "]= ("<< points[i].point << ").bestcenter "<< points[i].best_cluster << std::endl;
  }
  #endif
...
int main(int argc, char** argv) {
  ...
  //end timer
  duration = ( std::clock() - start ) / (double) CLOCKS_PER_SEC;
  std::cout<<"cluster center creation time "<< duration <<'\n';
